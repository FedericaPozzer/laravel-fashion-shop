...
<div class="container">
<h2 class="text-danger my-5">Modifica queste scarpe!</h2>

    <form action="{{ route("shoes.update", $shoe) }}" method="POST" class="row">
        @method("PUT")
        @csrf
        ...
        <div class="col-9 col-offset-5 my-3">
            ...
            <input type="url" class="form-control @error("immagine") is-invalid @enderror" id="immagine" name="immagine" value="{{ old("immagine") ?? $shoe->immagine }}">
            @error("immagine")
            <div class="invalid-feedback"> {{ $message }} </div>
            @enderror
        </div>
        ...
        <div class="col-12 my-3">
            ...
            <textarea class="border" name="descrizione" id="descrizione" rows="8" cols="80">{{ old("descrizione") ?? $shoe->descrizione }}</textarea>
            @error("descrizione")
                <div class="invalid-feedback"> {{ $message }} </div>
            @enderror
        </div>
